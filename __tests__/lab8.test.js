describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    let buttonText = await button.getProperty('textContent');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (i=1;i<prodItems.length;i++){
    // get the shadowRoot and query select the button inside, and click on it.
      let prodItem = prodItems[i];
      let prodShadowRoot = await prodItem.getProperty('shadowRoot');
      let button = await prodShadowRoot.$('button');
      //let button = await prodItem.$('shadow/.button')
      await button.click();
  }
    // Check to see if the innerText of #cart-count is 20
    let cartCount = await page.$('#cart-count');
    let cartCountText = await cartCount.getProperty('textContent');
    let cartCountTextValue = await cartCountText.jsonValue();
    expect(cartCountTextValue).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    let cartCountText = await cartCount.getProperty('textContent');
    let cartCountTextValue = await cartCountText.jsonValue();
    expect(cartCountTextValue).toBe('0');
  }, 10000);
